...
import static org.apache.lucene.codecs.sandbox.HnswBinaryQuantizedVectorsFormat.DIRECT_MONOTONIC_BLOCK_SHIFT;
 ...
 * @lucene.experimental
 */
public final class HnswBinaryQuantizedVectorsWriter extends KnnVectorsWriter {

  private static final long SHALLOW_RAM_BYTES_USED =
      RamUsageEstimator.shallowSizeOfInstance(HnswBinaryQuantizedVectorsWriter.class);
  ...
  public HnswBinaryQuantizedVectorsWriter(
    ...
    String metaFileName =
        IndexFileNames.segmentFileName(
            ...
            state.segmentSuffix,
            HnswBinaryQuantizedVectorsFormat.META_EXTENSION);

    String indexDataFileName =
        IndexFileNames.segmentFileName(
            state.segmentInfo.name,
            state.segmentSuffix,
            HnswBinaryQuantizedVectorsFormat.VECTOR_INDEX_EXTENSION);
    ...
    try {
      ...
      CodecUtil.writeIndexHeader(
          meta,
          HnswBinaryQuantizedVectorsFormat.META_CODEC_NAME,
          HnswBinaryQuantizedVectorsFormat.VERSION_CURRENT,
          state.segmentInfo.getId(),
          state.segmentSuffix);
      CodecUtil.writeIndexHeader(
          vectorIndex,
          HnswBinaryQuantizedVectorsFormat.VECTOR_INDEX_CODEC_NAME,
          HnswBinaryQuantizedVectorsFormat.VERSION_CURRENT,
          state.segmentInfo.getId(),
          state.segmentSuffix);
      success = true;
  ...
  public HnswGraph mergeOneFieldWithGraph(FieldInfo fieldInfo, MergeState mergeState) throws IOException {
    ...
    try {
      ...
      if (scorerSupplier.totalVectorCount() > 0) {
        ...
        graph =
            merger.merge(
                BinaryQuantizedFlatVectorsWriter.MergedQuantizedVectorValues
                    .mergeQuantizedBinaryVectorValues(fieldInfo, mergeState),
  ...
  private static class RAVectorValues implements RandomAccessVectorValues<long[]> {
    private final List<long[]> vectors;
    private final int dim;

    RAVectorValues(List<long[]> vectors, int dim) {
    ...
    @Override
    public RandomAccessVectorValues<long[]> copy() throws IOException {
      return this;
    }
  }
}
